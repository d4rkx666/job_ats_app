...
const PriceTable = ({currentPlan, plan, hadleOnClick, isLoading}) => {
        ...
        <div className="w-full mt-6">
          <div className="text-center">
            ...
            <button onClick={hadleOnClick} disabled={currentPlan || isLoading} className="w-full mt-4 py-2 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition disabled:bg-purple-300">
              {isLoading ? plan.ctaLoading : currentPlan ? t("currentPlan") : plan.cta}
            </button>
        </div>

        {/* Features list */}
        <div className="w-full mt-6 text-left">
          <p className="font-semibold text-gray-700">{t("includes")}</p>
            ...
            {plan.features.map((feature, i) => (
              <li key={i} className="flex items-center">
                {feature.included ? (
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" className="text-red-400 size-3 mr-3" viewBox="0 0 16 16" fill="currentColor">
