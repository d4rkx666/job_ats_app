...
import { UpgradeProKeywords } from '@/app/components/common/UpgradeProKeywords';
...
const CreateResumeForm = ({
  ...
  const f = useTranslations("formImproveResume");
  const {
    register,
    handleSubmit,
    trigger,
  ...
  const TEMPLATES = [
    { id: 'classic', name: t("form.templates.template1"), popular: false },
    { id: 'modern', name: t("form.templates.template2"), popular: true },
    { id: 'ats', name: t("form.templates.template3"), proOnly: true, badge: t("form.templates.bestScore") },
    { id: 'executive', name: t("form.templates.template4"), proOnly: true }
  ];
          ...
          <React.Fragment key={step}>
            <div className="flex flex-col items-center z-2">
              ...
              <span className={`text-sm mt-2 ${currentStep >= step ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>
                {[t("form.steps.step1.step"), t("form.steps.step2.step"), t("form.steps.step3.step"), t("form.steps.step4.step")][step - 1]}
              </span>
            </div>
            {step < 4 && (
              ...
              <div>
                ...
                <textarea
                  {...register('job_description', {
                    ...
                    minLength: {
                      value: 10,
                      message: t("form.jobDescriptionMinimumError")
                    }
                  })}
                ...
                <div className="flex justify-between items-center mt-1">
                  <span className="text-xs text-gray-500">{t("form.jobDescriptionMinimum")}</span>
                  {false && (
                    <button
                      type="button"
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center">
                ...
                <h2 className="text-2xl font-bold text-gray-800">
                  {draft.isOptimizedDraft ? t("form.steps.step2.title2") : t("form.steps.step2.title")}
                </h2>
              </div>
              {keywords?.length > 0 && (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                  {keywords.length} {t("form.steps.step2.keywordsAmount")}
                </span>
              )}
            </div>
            ...
            {(!keywords || keywords.length === 0) && (
              <div className="bg-white border-2 border-dashed border-gray-200 rounded-xl p-8 text-center">
                ...
                <h3 className="mt-2 text-lg font-medium text-gray-900">
                  {draft.isOptimizedDraft ? t("form.steps.step2.withoutKeywords") :t("form.steps.step2.subtitle")}
                </h3>
                <p className="mt-1 text-sm text-gray-500">
                  {draft.isOptimizedDraft
                    ? "This draft was created before keyword analysis was available."
                    : t("form.steps.step2.extractInfo")}
                </p>
                <div className="mt-6">
                <SubmitButton
                  ...
                  label={draft.isOptimizedDraft ? "Extract Keywords Now" : t("form.steps.step2.extractBtn")}
                  cost={costKeywords}
                  type={"button"}
            ...
            {keywords?.length > 0 && (
              ...
              <div className="flex justify-end">
                ...
                >
                    ...
                    {t("form.steps.step2.reExtractBtn")} (2 credits)
                </button>
              </div>
              </>
            )}
            ...
            <div className="space-y-5">
              <div className={`p-4 border rounded-lg ${isPro ? 'border-gray-200' : 'border-gray-100 bg-gray-50'}`}>
                <label className={`flex items-center ${!isPro ? 'opacity-75' : ''}`}>
                  ...
                  {t("form.steps.step4.coverLetter")}
                  </span>
                  <ProBadge className="ml-2" />
                </label>
                {isPro && (
                  <p className="text-sm text-gray-500 mt-2 ml-7">
                  {t("form.steps.step4.coverLetterInfo")}
                  </p>
                )}
                <p className="text-sm text-gray-500 mt-2 ml-7">
              ...
              <SubmitButton
                ...
                loading={isLoading}
                loadingLabel={t("form.steps.step4.creatingButton")}
                label={t("form.steps.step4.createButton")}
