...
function ResumeAdvisorForm({ onSubmit, isLoading, cost, submitButtonRef, error }) {
  // Translation
  const t = useTranslations("formImproveResume");
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-6 text-white">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div>
              ...
              <p className="mt-1 opacity-90">{t("subtitle")} <b>{creditsLeft} {t("credits")}</b></p>
            </div>
            <button
              type="button"
              onClick={() => setShowBenefits(!showBenefits)}
              className="mt-3 md:mt-0 px-4 py-2 bg-white/20 rounded-lg hover:bg-white/30 transition-all flex items-center"
            >
              {showBenefits ? t("hide") : t("show")}
              <svg
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm opacity-90">{t("step1.description")}</p>
                  </div>
                </div>
                <div className="flex items-start">
                  <div className="bg-white/20 p-2 rounded-full mr-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm opacity-90">{t("step2.description")}</p>
                  </div>
                </div>
                <div className="flex items-start">
                  <div className="bg-white/20 p-2 rounded-full mr-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm opacity-90">{t("step3.description")}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs text-gray-500">{t("resumeFile.max")}</span>
            </div>
              ...
              <label className="flex flex-col items-center px-4 py-8 cursor-pointer w-full">
                {filePreview ? (
                  <div className="text-center">
                    ...
                    <span className="block text-sm text-gray-500 mt-1">{t("resumeFile.change")}</span>
                  </div>
                  ...
                  <>
                    ...
                    <div className="text-center">
                      <span className="text-gray-500"> {t("resumeFile.placeholder")}</span>
                    </div>
                  </>
                )}
                <input
                  ...
                  id="resume"
                  {...register("resume", {
                    required: t("resumeFile.required"),
                    validate: () => {
                      if (!inputFileRef.current?.files?.[0]) return true;
                      const file = inputFileRef.current.files[0];
                      if (file.type !== "application/pdf") {
                        return t("resumeFile.invalidType");
                      }
                      if (file.size > 5 * 1024 * 1024) {
                        return t("resumeFile.invalidSize");
                      }
                      return true;
                    },
                  })}
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs text-gray-500">{t("eg")}</span>
            </div>
            ...
            <div className="relative">
              <textarea
                ...
                {...register("job_description", {
                  required: t("jobDescription.required"),
                })}
                rows="6"
                maxLength={3000}
          ...
          <div className="pt-2">
            <SubmitButton
              ...
              loading={isLoading}
              loadingLabel={t("uploadBtn.loading")}
              label={
                <div className="flex items-center justify-center">
                  <span>{t("uploadBtn.label")}</span>
...
export default ResumeAdvisorForm;