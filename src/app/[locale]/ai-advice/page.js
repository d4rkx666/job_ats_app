...
function AIAdvicePage() {
  // Language
  const t = useTranslations("previewImprovement");

  // get user
  const { user } = useAuth();

  // Check for feedback
  const hasFeedback = user.feedback;

  const { response_text, response_json = null, job_title } = useMemo(()=>{
    const data = JSON.parse(localStorage.getItem("ai-advice")) || "";
    localStorage.removeItem("ai-advice");
    return data;
  },[localStorage])
