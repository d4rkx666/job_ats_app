...
const VerifiEmailReminder = () => {
  ...
  // Language
  const t = useTranslations("verifyEmailReminder");
  const m = useTranslations("msg");

  const auth = useAuth();
  const [disabled, setDisabled] = useState(false);
  const [successful, setSuccessful] = useState("");
  const wait = process.env.NEXT_PUBLIC_TIME_EMAIL_RESEND; // in sec
  ...
  useEffect(()=>{
    if(counter.minutes === 0 && counter.seconds === 0){
      ...
      setSuccessful(m("emailSendAgain"));
      setDisabled(false);
    }
  });
    ...
    { !auth.verified && 
      <div className="text-center rounded-xl bg-white p-3 m-5 shadow-lg outline outline-black/5 dark:bg-slate-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">{t("subtitle")} {auth.user.email}</p>
