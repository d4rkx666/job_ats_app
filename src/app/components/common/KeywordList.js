...
export const KeywordList = ({isPro, keywords, profile = false}) => {
   // Translation
         ...
         <div className="relative group">
            ...
            {showCount && (
               <div className="absolute z-10 hidden group-hover:block bottom-full mb-2 px-2 py-1 text-xs font-medium text-white bg-gray-900 rounded whitespace-nowrap">
                  {t("appears")} {keyword.count} {t("time")}
               </div>
            )}
         </div>
      );
   ...
   return (
      <div className="space-y-8">
         ...
         {isPro && (
            <ProFeatureEnabled featureText={t("proEnabled")}/>
         )}
         ...
         <div className="space-y-6">
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden shadow-xs">
               <div className="bg-gray-50 px-5 py-3 border-b border-gray-100 flex items-center">
                  ...
                  <h3 className="font-medium text-gray-800">{t("coreRequirements")}</h3>
               ...
               <div className="p-5">
                  {keywords.filter(k => k.type === 'hard_skill').length > 0 ? (
                     ...
                     <p className="text-sm text-gray-500 italic">{t("noHardSkills")}</p>
                  )}
               </div>
            </div>
            ...
            {isPro && (
               <div className="border border-gray-100 rounded-xl overflow-hidden shadow-xs">
                  <div className="bg-gray-50 px-5 py-3 border-b border-gray-100 flex items-center">
                     ...
                     <h3 className="font-medium text-gray-800">{t("toolsAndTech")}</h3>
                  ...
                  <div className="p-5">
                     {keywords.filter(k => k.type === 'tool').length > 0 ? (
                        ...
                        <p className="text-sm text-gray-500 italic">{t("noTools")}</p>
                     )}
                  </div>
               </div>
            )}
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden shadow-xs">
               <div className="bg-gray-50 px-5 py-3 border-b border-gray-100 flex items-center">
                  ...
                  <h3 className="font-medium text-gray-800">{t("softSkills")}</h3>
               </div>
               <div className="p-5">
                  {keywords.filter(k => k.type === 'soft_skill').length > 0 ? (
                     ...
                     <p className="text-sm text-gray-500 italic">{t("noSoftSkills")}</p>
                  )}
               </div>
            </div>
            ...
            {isPro && keywords.filter(k => k.type === 'certification').length > 0 && (
               <div className="border border-gray-100 rounded-xl overflow-hidden shadow-xs">
                  <div className="bg-gray-50 px-5 py-3 border-b border-gray-100 flex items-center">
                     ...
                     <h3 className="font-medium text-gray-800">{t("certs")}</h3>
                     <ProBadge className="ml-auto" size="xs" />
                  </div>
                  <div className="p-5">
                     <div className="flex flex-wrap gap-3">
